...
function renderWordCount(root, toolId) {
  root.innerHTML = `
    <div class="panel">
      <div class="field-row"><label>Text</label><textarea id="wcIn" placeholder="Paste or type your text…" style="min-height:200px"></textarea></div>
...
function renderCaseConverter(root, toolId) {
  ...
  root.querySelectorAll('[data-case]').forEach((b) => b.addEventListener('click', () => {
    ...
    else if (c === 'camel') out = t.toLowerCase().replace(/[^a-z0-9]+(.)/g, (_, ch) => ch.toUpperCase()).replace(/^[A-Z]/, (ch) => ch.toLowerCase());
  ...
  root.querySelector('#ccCopy').addEventListener('click', () => {
    navigator.clipboard.writeText(root.querySelector('#ccIn').value);
    toast('Copied');
  });
}

/* ============================================================
   Line Cleaner (dedupe / sort / trim / reverse / etc)
   ============================================================ */
function renderTextCleaner(root, toolId) {
  root.innerHTML = `
    <div class="panel">
      <div class="field-row"><label>Text</label><textarea id="tcIn" placeholder="Paste text…" style="min-height:200px"></textarea></div>
      <div class="field-group">
        ...
        <button class="btn btn-outline btn-sm" data-op="sort">Sort A→Z</button>
        <button class="btn btn-outline btn-sm" data-op="rsort">Sort Z→A</button>
      ...
      <div class="actions">
        <button id="tcCopy" class="btn btn-primary">Copy result</button>
      </div>
    </div>`;
  ...
  root.querySelector('#tcCopy').addEventListener('click', () => {
    navigator.clipboard.writeText(root.querySelector('#tcIn').value);
    toast('Copied');
  });
}
...
function renderFindReplace(root, toolId) {
  ...
  root.querySelector('#frCopy').addEventListener('click', () => {
    navigator.clipboard.writeText(root.querySelector('#frIn').value);
    toast('Copied');
  });
}
...
function renderTextToPdf(root, toolId) {
  root.innerHTML = `
    <div class="panel">
      <div class="field-row"><label>Text</label><textarea id="tpIn" placeholder="Enter text to convert to PDF…" style="min-height:220px"></textarea></div>
      <div class="tool-row">
      ...
      <div class="actions">
        <button id="tpGo" class="btn btn-primary">${icon('file', 16)} Generate PDF</button>
      </div>
    </div>`;
...
function renderJsonFormatter(root, toolId) {
  root.innerHTML = `
    <div class="panel">
      <div class="field-row"><label>JSON</label><textarea id="jfIn" placeholder='{"hello": "world"}' style="min-height:220px"></textarea></div>
      <div class="field-group">
  ...
  root.querySelector('#jfFormat').addEventListener('click', () => {
    try {
      root.querySelector('#jfIn').value = JSON.stringify(JSON.parse(root.querySelector('#jfIn').value), null, 2);
  ...
  root.querySelector('#jfMinify').addEventListener('click', () => {
    try {
      root.querySelector('#jfIn').value = JSON.stringify(JSON.parse(root.querySelector('#jfIn').value));
  ...
  root.querySelector('#jfCopy').addEventListener('click', () => {
    navigator.clipboard.writeText(root.querySelector('#jfIn').value);
    toast('Copied');
  });
}
...
function renderJsonCsv(root, toolId) {
  root.innerHTML = `
    <div class="panel">
      <div class="field-row"><label>Input (JSON array of objects, or CSV)</label><textarea id="jcIn" placeholder='[{"name":"Alice","age":30}]' style="min-height:200px"></textarea></div>
      <div class="field-group">
  ...
  root.querySelector('#jcCopy').addEventListener('click', () => {
    navigator.clipboard.writeText(root.querySelector('#jcOut').textContent);
    toast('Copied');
  });
...
function renderMarkdownHtml(root, toolId) {
  ...
  root.querySelector('#mhToHtml').addEventListener('click', () => {
  ...
  root.querySelector('#mhCopy').addEventListener('click', () => {
    navigator.clipboard.writeText(root.querySelector('#mhOut').textContent);
    toast('Copied');
  });
}

/* ============================================================
   Export registry — THIS is what app.js imports
   ============================================================ */
